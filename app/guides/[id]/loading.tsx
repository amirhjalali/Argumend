      ...
      <article className="mx-auto max-w-3xl px-4 md:px-8 py-8 md:py-14">
        ...
        <header className="mb-12 pb-8 border-b border-stone-200/60">
          <div className="flex items-center gap-3 mb-5">
            <Skeleton className="w-12 h-12" radius="rounded-xl" />
            <div className="flex items-center gap-3">
              <SkeletonBadge width="w-28" className="h-5" />
          ...
          {/* Subtitle */}
          <SkeletonParagraph lines={2} className="[&>div]:h-6" />
        </header>
        ...
        {Array.from({ length: 3 }).map((_, sectionIdx) => (
          <section key={sectionIdx} className="mb-12">
            ...
            {/* Section content */}
            <SkeletonParagraph lines={4} className="mb-6" />
          ...
          <ul className="space-y-3">
            {Array.from({ length: 4 }).map((_, idx) => (
              <li key={idx} className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 bg-stone-200/60 rounded-full mt-2.5 flex-shrink-0" />
