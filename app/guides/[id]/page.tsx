          ...
          <section className="my-12">
            ...
            <div className="space-y-2">
              {guide.furtherReading.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-start justify-between gap-3 rounded-xl border border-stone-200/60 bg-white/80 p-4 transition-all duration-200 hover:border-deep/30 hover:shadow-sm dark:border-[var(--border-default)] dark:bg-card/80 dark:hover:border-teal-400/50 sm:items-center"
                >
              ...
              const renderCard = (otherGuide: (typeof guides)[number], idx: number) => {
                ...
                return (
                  <Link
                    ...
                    className={`group flex items-center gap-3 p-4 rounded-xl bg-card/80 border border-stone-200/60 dark:border-[var(--border-default)] hover:shadow-sm hover:-translate-y-0.5 transition-all duration-200 animate-card-fade-in ${otherTrack.hoverBorder}`}
