          ...
          <div className="bg-white/80 rounded-xl border border-[#e8e0d4] overflow-hidden">
            <table className="w-full">
              <thead className="bg-gradient-to-b from-[#f8f5f0] to-[#f4f1eb]">
                <tr>
                  <th className="text-left px-5 py-3 text-sm font-semibold text-primary">Topic</th>
                  <th className="text-left px-5 py-3 text-sm font-semibold text-primary hidden sm:table-cell">Pillars</th>
                  <th className="text-right px-5 py-3 text-sm font-semibold text-primary">Confidence</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#e8e0d4]">
                {topics.map((topic) => (
                  <tr key={topic.id} className="hover:bg-[#faf7f2] transition-colors">
                    ...
                    <td className="px-5 py-3 text-sm md:text-base text-secondary hidden sm:table-cell">
                      {topic.pillars.map(p => p.title).join(", ")}
                    </td>
                    <td className="px-5 py-3 text-right font-mono text-sm md:text-base text-primary">
              ...
              <div key={category.category}>
                ...
                <div className="space-y-2">
                  {category.resources.map((resource) => (
                    ...
                    >
                      <div>
                        <h4 className="font-medium text-primary group-hover:text-deep transition-colors">{resource.title}</h4>
