...
- **10 of the 13 settled topics lose "settled" if any one card is relabelled.**
  ...
  `affirmative-action-meritocracy` survive, and each of those survives by
  exactly two flips — none by three.
...
> A map displays **settled** only when `flipsToChange >= 2` (no single card
> relabel can take the word away) **and** it carries at least **8** evidence
> cards. Otherwise the quadrant is demoted to `moderate`, the label becomes the
> lean alone (`getLeanLabel`, e.g. "Clearly favors the claim"), and
...
2. **It only ever demotes.** `contested` and `open` are untouched. The rule can
   remove a claim the evidence cannot carry; it can never manufacture one.
3. **It is honest about itself.** A demoted verdict carries `fragile: true`, and
   every surface that shows the label shows one line under it: *"One evidence
   card could change this reading."* A silent demotion would trade one
   misleading display for another.
...
It would demote 11 of the 13 rather than 10 — including
...
156 topics. **13 settled before → 3 after; 10 topics change quadrant.**
...
| `moon-landing` | 8 | 76 | 82 | settled | moderate | 1 | 61–88 |
...
Surviving as settled: `affirmative-action-meritocracy` (14 cards, balance 78,
2 flips), `animal-consciousness-rights` (11, 81, 2), `climate-change` (12, 85, 2).
...
Library-wide on the corrected data: **17 settled before → 5 after; 12 topics
demoted** (the 10 above plus `us-iran-conflict` and `doge-federal-cuts`).
`flipsToChange` among the 17 settled-before: **1** → 12 · **2** → 4 · **3** → 1.
...
1. **`moon-landing` is demoted, and that is the decision to make.** Its map is
   8 cards — three "for" plus one steelmanned "against" per pillar, two pillars —
   which puts balance at 76, six points over the line. Relabelling either
   "against" card drops it to 61. The reading is true of *this map* and false of
   the world, and the site will read "Clearly favors the claim" about the moon
   landing until one of three things happens: (a) the map gains cards — it is the
   flagship and should carry 12+, and this is the recommended fix; (b) topics
   authored `status: "settled"` are exempted from the guard, which is a one-topic
   carve-out today since `climate-change` is the only other one and it passes
...
5. **Is `fragile` the right thing to persist?** It is currently derived at build
   time and stored on the verdict. The underlying `flipsToChange` and
   `oneCardBalanceRange` are not stored; anything that wants them recomputes via
   `topicVerdictSensitivity`. If a surface ever wants to show "2 cards from
   changing", that decision gets revisited.
...
| topic | cards | balance | weight | before | after | flips | one-card range | fragile |
| --- | ---: | ---: | ---: | --- | --- | ---: | --- | --- |
...
| `moon-landing` | 8 | 76 | 82 | settled | moderate | 1 | 61–88 (27) | **yes** |
